...
class _SfCalendarState extends State<SfCalendar>
      ...
      for (int i = 0; i < _forwardWidgetHeights.length; i++) {
        ...
        if (scrolledPosition >= widgetPosition &&
          ...
          if (interSectionPoint != -1 &&
            ...
            date = addDays(date, 6) as DateTime;
          }
      ...
      case CalendarDataSourceAction.add:
        {
          final List<CalendarAppointment> collection =
              AppointmentHelper.generateCalendarAppointments(widget.dataSource,
                  widget.timeZone, data as List<CalendarAppointment>);
      ...
      case CalendarDataSourceAction.remove:
        {
          for (int i = 0; i < data.length; i++) {
            final Object appointment = data[i] as Object;
  ...
  Map<DateTime, List<CalendarAppointment>> _getAppointmentCollectionOnDateBasis(
    ...
    while (startDate.isBefore(endDate) || isSameDate(endDate, startDate)) {
      ...
      startDate = addDays(startDate, 1) as DateTime;
    }
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL) {
          ...
          while (count < 20) {
            for (int i = 1; i <= 100; i++) {
              final DateTime updateDate =
                  addDays(date, i * DateTime.daysPerWeek) as DateTime;
              ...
              final DateTime weekEndDate = addDays(updateDate, 6) as DateTime;
            ...
            date = addDays(date, 700) as DateTime;
          }
        }
      } else {
        ...
        if (_previousDates.isNotEmpty && -index > _previousDates.length - 20) {
          ...
          while (count < 20) {
            for (int i = 1; i <= 100; i++) {
              ...
              final DateTime weekEndDate = addDays(updatedDate, 6) as DateTime;
    ...
    final DateTime prevEndDate = addDays(prevDate, 6) as DateTime;
    final DateTime endDate = addDays(startDate, 6) as DateTime;
    ...
    Color dividerColor = widget.cellBorderColor ??
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    if (_nextDates.isEmpty) {
      ...
      DateTime date = addDays(scheduleDisplayDate, value) as DateTime;
      int count = 0;
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      DateTime viewEndDate =
          addDays(_nextDates[_nextDates.length - 1], 6) as DateTime;
      List<CalendarAppointment> appointmentCollection =
          AppointmentHelper.getVisibleAppointments(
              viewStartDate,
      ...
      if (_useMobilePlatformUI) {
        DateTime previousDate = addDays(viewStartDate, -1) as DateTime;
        for (int i = 0; i < _nextDates.length; i++) {
      ...
      while (totalHeight < height &&
        ...
        if (totalHeight != 0) {
          ...
          viewEndDate = addDays(currentDate, 6) as DateTime;
    ...
    if (_nextDates.isNotEmpty &&
      ...
      final DateTime viewEndDate = addDays(viewStartDate, 6) as DateTime;
      if (viewStartDate.isBefore(scheduleDisplayDate) &&
        ...
        final DateTime viewEndDate =
            addDays(scheduleDisplayDate, -1) as DateTime;
  ...
  Widget addAgendaWithLoadMore(double height, bool isRTL) {
    ...
    if (_nextDates.isNotEmpty &&
      ...
      while (isSameOrBeforeDate(_maxDate, viewStartDate)) {
        final DateTime viewEndDate =
            addDays(viewStartDate, DateTime.daysPerWeek - 1) as DateTime;
        ...
        viewStartDate =
            addDays(viewStartDate, DateTime.daysPerWeek) as DateTime;
      }
      ...
      for (int i = 0; i < _nextDates.length; i++) {
        ...
        final DateTime visibleEndDate =
            addDays(visibleStartDate, DateTime.daysPerWeek) as DateTime;
      ...
      while (viewStartDate.isBefore(scheduleDisplayDate) &&
        ...
        if (appEndDate.isAfter(scheduleDisplayDate) ||
          ...
          appEndDate = addDays(scheduleDisplayDate, -1) as DateTime;
        }
  ...
  Widget _addDatePicker(double top, bool isRTL) {
    ...
    final TextStyle datePickerStyle =
        widget.monthViewSettings.monthCellStyle.textStyle ??
            (_calendarTheme.activeDatesTextStyle ?? const TextStyle());
    final Color? todayColor =
        widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor;
    double left = 0;
    if (_isMobilePlatform) {
