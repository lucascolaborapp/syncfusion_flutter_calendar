...
class _MonthViewRenderObject extends CustomCalendarRenderObject {
  ...
  void _drawMonthCells(Canvas canvas, Size size) {
    ...
    TextStyle textStyle = monthCellStyle.textStyle ??
        (calendarTheme.activeDatesTextStyle ?? const TextStyle());
    final int visibleDatesCount = visibleDates.length;
    final DateTime currentMonthDate = visibleDates[visibleDatesCount ~/ 2];
    final int nextMonth =
        (getNextMonthDate(currentMonthDate) as DateTime).month;
    final int previousMonth =
        (getPreviousMonthDate(currentMonthDate) as DateTime).month;
    final DateTime today = DateTime.now();
    bool isCurrentDate;

    _linePainter.isAntiAlias = true;
    final TextStyle todayStyle =
        todayTextStyle ?? (calendarTheme.todayTextStyle ?? const TextStyle());
    final TextStyle currentMonthTextStyle = monthCellStyle.textStyle ??
        (calendarTheme.activeDatesTextStyle ?? const TextStyle());
    final TextStyle previousMonthTextStyle =
        monthCellStyle.trailingDatesTextStyle ??
            (calendarTheme.trailingDatesTextStyle ?? const TextStyle());
    final TextStyle nextMonthTextStyle = monthCellStyle.leadingDatesTextStyle ??
        (calendarTheme.leadingDatesTextStyle ?? const TextStyle());
    ...
    for (int i = 0; i < visibleDatesCount; i++) {
      ...
      if (currentVisibleDate.month == nextMonth) {
        ...
        _linePainter.color = monthCellStyle.leadingDatesBackgroundColor ??
            (calendarTheme.leadingDatesBackgroundColor ?? Colors.black);
        ...
        _linePainter.color = monthCellStyle.trailingDatesBackgroundColor ??
            (calendarTheme.trailingDatesBackgroundColor ?? Colors.black);
      } else {
        textStyle = currentMonthTextStyle;
        _linePainter.color = monthCellStyle.backgroundColor ??
            (calendarTheme.activeDatesBackgroundColor ?? Colors.black);
      }
      ...
      if (isSameDate(currentVisibleDate, today)) {
        _linePainter.color = monthCellStyle.todayBackgroundColor ??
            (calendarTheme.todayBackgroundColor ?? Colors.black);
  ...
  void _drawVerticalAndHorizontalLines(Canvas canvas, Size size,
    ...
    _linePainter.color =
        cellBorderColor ?? (calendarTheme.cellBorderColor ?? Colors.black);
